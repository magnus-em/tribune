...
// NOTE: do NOT build this with `.merge()` of the per-step schemas. In zod v4,
// merging ZodEffects (schemas carrying a `superRefine`) silently drops the
// refinements, so the server-side `intakeSchema.safeParse` in createCase would
// skip every cross-field check (landlord contact required, withheld ≤ deposit,
// deposit > 0, yes/no answered). The server is the authority, so we re-declare
// the full object and attach one combined refinement here.
export const intakeSchema = z
  ...
  .superRefine((data, ctx) => {
    if (data.lease_start_date && data.lease_end_date) {
      if (new Date(data.lease_start_date) >= new Date(data.lease_end_date)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Lease end must be after start", path: ["lease_end_date"] });
      }
    }
    if (!data.landlord_email?.trim() && !data.landlord_phone?.trim()) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "At least one contact method is required (email or phone)", path: ["landlord_email"] });
    }
    const deposit = parseFloat(data.deposit_amount);
    const withheld = parseFloat(data.amount_withheld);
    if (isNaN(deposit) || deposit <= 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Enter a valid deposit amount", path: ["deposit_amount"] });
    }
    if (isNaN(withheld) || withheld < 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Enter a valid amount", path: ["amount_withheld"] });
    }
    if (!isNaN(deposit) && !isNaN(withheld) && withheld > deposit) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Amount withheld cannot exceed deposit", path: ["amount_withheld"] });
    }
    if (!data.notice_given) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Please select one", path: ["notice_given"] });
    }
